	...
	function _initialize() {
		...
		if($ucenter_syn){
			...
			if(empty($session_user)){
				if(!empty($_COOKIE['thinkswn_auth'])  && $_COOKIE['thinkswn_auth']!="logout"){
					$thinkswn_auth=sp_authcode($_COOKIE['thinkswn_auth'],"DECODE");
					$thinkswn_auth=explode("\t", $thinkswn_auth);
					$auth_username=$thinkswn_auth[1];
					$users_model=M('Users');
