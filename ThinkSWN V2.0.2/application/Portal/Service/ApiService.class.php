...
class ApiService {
     ...
     * 功能:查询标签列表,支持分页;
     */
    public static function tags($tag,$where=array(),$pagesize=0,$pagetpl=''){
		$tags_model=M('tags');
    	$where=is_array($where)?$where:array();
    	$tag=sp_param_lable($tag);
    ...
    /**
     * 根据标签ID 获取该标签下所有文章,已经分页
     */
    public static function postsPagedByTagId($tag_id,$tag,$pagesize=20,$pagetpl=''){
        $tag_id=intval($tag_id);
        $post_ids=array();
        $post_ids=M("Tags")->where(array('id'=>$tag_id))->getField('post_id');
