...
class AdminPostController extends AdminbaseController {
	...
	public function article_index(){
		$this->_lists(array("post_status"=>array('neq',3),'type'=>'article'));
		$this->_getTree();
		$this->display();
	}
	// 后台问题管理列表
	public function question_index(){
		$this->_lists(array("post_status"=>array('neq',3),'type'=>'question'));
		$this->_getTree();
		$this->display();
	}
	// 后台答案管理列表
	public function answer_index(){
		...
		if(!empty($term_id)){
		    $this->answers_model->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		
		$count=$this->answers_model->count();
	...
	public function add_post(){
		if (IS_POST) {
			foreach ($_POST['term'] as $mterm_id){
				$terms_exist=$this->terms_model->where(array("term_id"=>intval($mterm_id)))->find();
			}
	...
	public function answer_delete(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			$isbest=$this->answers_model->where(array('id'=>$id))->getField('isbest');
			if(!$isbest){
				if ($this->answers_model->where(array('id'=>$id))->delete()) {
					$this->success("删除成功！");
				} else {
					$this->error("删除失败！");
				}
			}else{
				$this->error("被采纳答案不能删除！");
			}
		}
	}
	// 审核
