...
class IndexadminController extends AdminbaseController {
    ...
    public function expert(){
    	$id= I('get.id',0,'intval');
    	if ($id) {
    		$result = M("Users")->where(array("id"=>$id,"user_type"=>2))->setField('expert',1);
    		if ($result) {
    			$this->success("设置成专家成功！", U("indexadmin/index"));
    		} else {
    			$this->error('设置成专家失败,会员不存在,或者是管理员！');
    		}
    	} else {
    		$this->error('数据传入失败！');
    	}
    ...
    public function cancelexpert(){
    	$id= I('get.id',0,'intval');
    	if ($id) {
    		$result = M("Users")->where(array("id"=>$id,"user_type"=>2))->setField('expert',0);
    		if ($result) {
    			$this->success("取消专家成功！", U("indexadmin/index"));
    		} else {
    			$this->error('取消专家失败,会员不存在,或者是管理员！');
    		}
    	} else {
    		$this->error('数据传入失败！');
    	}
